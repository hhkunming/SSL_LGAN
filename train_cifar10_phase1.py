...
parser.add_argument('--AE_weight', type=float, default=100.)
...
z_jacobian = T.matrix()
temp = ll.get_output(gen_layers[-1], {gen_img_input: x, gen_noise_input: noise}, deterministic=False, init=True)
...
train_batch_disc = th.function(inputs=[x_lab,labels,x_unl,x,lr], outputs=[loss_lab, loss_unl, train_err], updates=disc_param_updates+disc_avg_updates)
...
gen_param_updates = nn.adam_updates(gen_params, loss_gen+args.AE_weight*loss_ae+args.gen_lab_weight*loss_gen_lab, lr=lr, mom1=0.5)
train_batch_gen = th.function(inputs=[x_unl,x,x_lab,labels,lr], outputs=[loss_gen, loss_ae, loss_gen_lab], updates=gen_param_updates)
...
for epoch in range(1200):
    ...
    for t in range(nr_batches_train):
        ...
        lg, lae, lp = train_batch_gen(trainx_unl2[t*args.batch_size:(t+1)*args.batch_size],trainx_unl3[t*args.batch_size:(t+1)*args.batch_size],
    ...
    # report
    print("Iteration %d, time = %ds, loss_lab = %.4f, loss_unl = %.4f, loss_gen = %.4f, loss_ae = %.4f, loss_prv = %.4f, train err = %.4f, test err = %.4f" % (epoch, time.time()-begin, loss_lab, loss_unl, loss_gen, loss_ae, loss_prv, train_err, test_err))
    sys.stdout.flush()

    # generate samples from the model
    if test_err < best_err:
        print 'Best found! Saving...'
        np.savez(args.save_dir+'disc_avg_params_phase1.npz', *[p.get_value() for p in disc_param_avg])
        np.savez(args.save_dir+'disc_params_phase_1.npz', *lasagne.layers.get_all_param_values(disc_layers))
        np.savez(args.save_dir+'gen_params_phase1.npz', *lasagne.layers.get_all_param_values(gen_layers))
