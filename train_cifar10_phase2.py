...
parser.add_argument('--gen_jacobi_weight', type=float, default=0.001)
parser.add_argument('--disc_lap_weight_lab', type=float, default=0.01)
parser.add_argument('--disc_lap_weight_unl', type=float, default=0.001)
parser.add_argument('--z_delta', type=float, default=0.1)
parser.add_argument('--learning_rate', type=float, default=0.0003)
parser.add_argument('--start_epoch', type=int, default=0)
parser.add_argument('--data_dir', type=str, default='/path/to/data/cifar-10-python')
parser.add_argument('--phase1_model_dir', type=str, default='/path/to/phase1_model_dir')
...
# load models
with np.load(args.phase1_model_dir+'/disc_params_phase1.npz') as f:
    param_values = [f['arr_%d' % i] for i in range(len(f.files))]
ll.set_all_param_values(disc_layers, param_values)

with np.load(args.phase1_model_dir+'/gen_params_phase1.npz') as f:
    param_values = [f['arr_%d' % i] for i in range(len(f.files))]
ll.set_all_param_values(gen_layers, param_values)

with np.load(args.phase1_model_dir+'/disc_avg_params_phase1.npz') as f:
    disc_param_avg = [th.shared(np.cast[th.config.floatX](f['arr_%d' % i])) for i in range(len(f.files))]
...
# //////////// perform training //////////////
for epoch in range(args.start_epoch, 1200):
    begin = time.time()
    ...
    if test_err < best_err:
        ...
        np.savez(args.save_dir+'/disc_avg_params_phase2.npz', *[p.get_value() for p in disc_param_avg])
        np.savez(args.save_dir+'/disc_params_phase2.npz', *lasagne.layers.get_all_param_values(disc_layers))
        np.savez(args.save_dir+'/gen_params_phase2.npz', *lasagne.layers.get_all_param_values(gen_layers))
